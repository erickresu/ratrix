...
class RateWizardBloc extends Bloc<RateWizardEvent, RateWizardState> {
  RateWizardBloc({
    ...
    RatrixRate? existingRate,
        ...
        super(
          existingRate != null
              ? _buildStateFromExistingRate(existingRate, isCustom: isCustom, clientId: clientId, clientName: clientName)
              : RateWizardState(isCustom: isCustom, clientId: clientId, clientName: clientName),
        ) {
    ...
    on<BreakweightAdded>((event, emit) {
      final prevMax = state.breakweights.isNotEmpty ? num.tryParse(state.breakweights.last.max) : null;
      final nextMin = prevMax != null ? _numToString(prevMax + 1) : '1';
      emit(state.copyWith(
        breakweights: [...state.breakweights, Breakweight(min: nextMin)],
    ...
    on<ConditionalBreakweightAdded>((event, emit) {
      final prevMax = state.conditionalBreakweights.isNotEmpty ? num.tryParse(state.conditionalBreakweights.last.max) : null;
      final nextMin = prevMax != null ? _numToString(prevMax + 1) : '1';
      emit(state.copyWith(
        conditionalBreakweights: [...state.conditionalBreakweights, Breakweight(min: nextMin)],
  ...
  Future<void> _onSubmitRequested(RateSubmitRequested event, Emitter<RateWizardState> emit) async {
    ...
    for (var i = 0; i < state.matrixRows.length; i++) {
      ...
      final key = '${row.origin.trim().toLowerCase()}|${row.destination.trim().toLowerCase()}';
      if (key == '|') continue; // both blank — not a meaningful duplicate.
      if (!seenRoutes.add(key)) {
        emit(state.copyWith(
          isSubmitting: false,
          submitError: 'Route ${i + 1} has the same origin and destination as another row. '
      ...
      rows: [
        for (final row in state.matrixRows)
          (
            ...
            original: row.routeId != null ? originalRoutesById[row.routeId]?.toJson() : null,
            routeId: row.routeId,
    ...
    try {
      ...
      emit(state.copyWith(isSubmitting: false, submitSucceeded: true, lastSubmitStayedOnPage: event.stayOnPage));
  ...
  static RateWizardState _buildStateFromExistingRate(
    ...
    final freightMode = rate.freightMode?.id != null ? RatesFkIds.freightModeFromId[rate.freightMode!.id] : null;
    final serviceMode = (rate.serviceMode?.id != null ? RatesFkIds.serviceModeFromId[rate.serviceMode!.id] : null) ??
        ServiceMode.doorToDoor;
    final chargeBasis =
        (rate.chargeBasis?.id != null ? RatesFkIds.chargeBasisFromId[rate.chargeBasis!.id] : null) ?? ChargeBasis.kilo;
    ...
    final derivedPrefix = '${(freightMode?.name ?? '').toUpperCase()}_${serviceMode.abbreviation}';
    ...
    final firstRouteBreakweights = rate.routes.isNotEmpty ? rate.routes.first.breakweights : const <RatrixBreakweight>[];
    final breakweights = firstRouteBreakweights.isNotEmpty
        ? [for (final bw in firstRouteBreakweights) Breakweight(min: _numToString(bw.min), max: _numToString(bw.max))]
        : const [Breakweight()];
    ...
    if (addons != null) {
      ...
      // freight mode.
      final thc = freightMode == FreightMode.sea ? addons.seaThc : addons.airThc;
      put('thc', thc);

      if (addons.fuelSurchargeType != null && addonValues.containsKey('fuel')) {
        addonModes['fuel'] = addons.fuelSurchargeType == 'percentage' ? AddonMode.percentage : AddonMode.exact;
      }
      if (addons.valuationType != null && addonValues.containsKey('valuation')) {
        addonModes['valuation'] = addons.valuationType == 'percentage' ? AddonMode.percentage : AddonMode.exact;
      }
    }
  ...
  static String _findMatchingRate(List<RatrixBreakweight> routeBreakweights, RatrixBreakweight slot) {
    for (final bw in routeBreakweights) {
      if (bw.min == slot.min && bw.max == slot.max) return _numToString(bw.rate);
    }
    return '';
  }
